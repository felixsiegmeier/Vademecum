...
from .schema import TherapieOutput
...
def _render_therapie(data: TherapieOutput) -> str:
    lines: list[str] = []
    if data.initial_op:
        lines.append("**Initial-OP:**")
        lines.append(data.initial_op)
        lines.append("")
    if data.antimikrobiell:
        lines.append("**Antimikrobielle Therapie:**")
        for item in data.antimikrobiell:
            lines.append(f"- {item}")
        lines.append("")
    if data.weitere:
        lines.append("**Weitere Prozeduren:**")
        for item in data.weitere:
            lines.append(f"- {item}")
    return "\n".join(lines).strip()
...
async def run(
    ...
    """Generates the Therapie section of an Arztbrief from a Patient object.
    ...
    data = TherapieOutput.model_validate_json(raw)
